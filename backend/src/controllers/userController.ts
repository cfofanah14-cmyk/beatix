...
export async function getProfile(req: AuthRequest, res: Response): Promise<void> {
  try {
    const r = await query<UserRow>(
      'SELECT id,full_name AS name,email,phone,avatar_url,role,created_at FROM users WHERE id=$1',
      [req.user!.userId]
    );
  ...
  try {
    const r = await query<UserRow>(
      `UPDATE users SET full_name=COALESCE($1,full_name), phone=COALESCE($2,phone), updated_at=NOW()
       WHERE id=$3 RETURNING id,full_name AS name,email,phone,avatar_url,role`,
