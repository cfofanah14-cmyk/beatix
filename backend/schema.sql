...
CREATE TYPE notification_type AS ENUM ('confirmation', 'reminder', 'cancellation', 'payout', 'system');
...
CREATE TABLE users (
  ...
  avatar_url        TEXT,
  language          VARCHAR(10) NOT NULL DEFAULT 'en',  -- 'en' | 'krio'
...
CREATE TABLE otp_codes (
  ...
  purpose     VARCHAR(30) NOT NULL DEFAULT 'login',  -- 'login' | 'register' | '2fa'
...
-- ── STAFF (scanner / entry roles) ────────────────────────────────────────────

CREATE TABLE staff (
  id            UUID        PRIMARY KEY DEFAULT uuid_generate_v4(),
  organizer_id  UUID        NOT NULL REFERENCES organizers(id) ON DELETE CASCADE,
  user_id       UUID        NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  role          VARCHAR(30) NOT NULL DEFAULT 'scanner',  -- 'scanner' | 'manager'
...
CREATE TABLE ticket_types (
  ...
  name            VARCHAR(80)   NOT NULL,  -- 'VIP', 'Regular', 'Student', etc.
  description     TEXT,
...
CREATE TABLE transactions (
  ...
  payment_reference   VARCHAR(100),       -- Flutterwave transaction ref
...
-- ── TICKETS (individual QR-code tickets) ─────────────────────────────────────
...
CREATE TABLE tickets (
  ...
  ticket_type_id  UUID            NOT NULL REFERENCES ticket_types(id),
  user_id         UUID            REFERENCES users(id),
  ticket_number   VARCHAR(30)     NOT NULL UNIQUE,  -- BTX-2025-XXXX-XXXXX
  qr_code_data    TEXT            NOT NULL UNIQUE,  -- signed token for scan
...
CREATE TABLE payouts (
  ...
  method          VARCHAR(30)   NOT NULL,  -- 'mobile_money' | 'bank'
...
CREATE TABLE reports (
  ...
  target_type   VARCHAR(30) NOT NULL,  -- 'event' | 'organizer' | 'transaction'
  target_id     UUID        NOT NULL,
  reason        TEXT        NOT NULL,
  status        VARCHAR(20) NOT NULL DEFAULT 'open',  -- 'open' | 'reviewed' | 'closed'
  created_at    TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
-- Event summary with organizer info (used by discovery page)
